...
const getData = async () => {
  const response = await axios
    .get(`http://127.0.0.1:3000/api/topics_alldata?_id=${route.query._id}`)
    .then((res) => {
      data.value = res.data;
    })
    .catch((err) => {
...
const submitForm = async () => {
  ...
  const response = await axios
    .patch(
      `http://127.0.0.1:3000/api/topics_updata_one?_id=${route.query._id}`,
      data.value
    )
    .then((res) => {
      alert("已提交成功l ！");
  ...
  <div>
    <form>
      ...
      <div class="container">
        <div class="tianjia">
          <h2 class="myh2">论题</h2>
          <n-input v-model:value="data.title" type="text" placeholder="论题" />
